...
class EnergyScraper(object):
  def run(self, options):
    ...
    for url in self.urls_for():
      page = BeautifulSoup(utils.download(url), "lxml")
  ...
  def fetch_from_landing_page(self, landing_url):
    ...
    unreleased = False
    page = BeautifulSoup(utils.download(landing_url), "lxml")
  ...
  def urls_for(self):
    ...
    # reports.
    page = BeautifulSoup(utils.download(BASE_URL), "lxml")
    ...
    for li in page.select('.field-items li'):
      ...
      if md:
        ...
        if cur_year >= self.year_range[0] and cur_year <= self.year_range[-1]:
          ...
          # pages enumerated.
          next_page = BeautifulSoup(utils.download(next_url), "lxml")
